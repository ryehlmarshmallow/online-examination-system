...
public class AttemptAnswerVisibilityService {
    ...
    public boolean canStudentViewRubric(ExamAttempt attempt) {
        Instant startTime = attempt.getExam().getStartTime();
        if (startTime != null && Instant.now().isBefore(startTime)) {
            return false;
        }

        StudentAnswerVisibilityMode mode = attempt.getExam().getStudentAnswerVisibilityMode();
        if (mode == null) {
            mode = StudentAnswerVisibilityMode.VIEW_AFTER_FINISHED_EACH_ATTEMPT;
        }
